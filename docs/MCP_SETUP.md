...
This guide explains how to configure and use MCP servers with your AI agent for browser automation and test execution in this project.

---

## What is MCP?

**Model Context Protocol (MCP)** allows your AI agent to interact with external tools and servers. Instead of only reading and writing files, your AI agent can use MCP to control a browser, run tests, and interact with other services directly.
...
| **your AI agent** | CLI or IDE extension for your preferred AI tool |
...
> **Note:** your AI agent also works from the terminal without VS Code. The extensions are recommended but not required.
...
> **Important:** The `.mcp.json` file **must** live in the repository root (the same directory as `package.json`). your AI agent looks for it in the current working directory when it starts. If it is in a subdirectory, MCP servers will not be detected.
...
| `playwright` | Server name -- used to identify this server in your AI agent |
...
**Step 2 — Start your AI agent in the project directory:**

```bash
cd /path/to/your-repo
# Start your AI agent according to its specific CLI command
```

**Step 3 — Ask your AI agent to list MCP tools:**
...
## Using MCP with your AI agent

### Browser Automation (playwright server)

The `playwright` MCP server gives your AI agent direct control over a browser. Use it for:
...
The `playwright-test` MCP server lets your AI agent run your Playwright tests and analyze results. Use it for:
...
### MCP tools are not appearing in your AI agent

1. Restart your AI agent (close and reopen in the project directory).
